...
export class DocumentController {
  public getByOwner = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const documents = await service.getDocumentsByOwner(String(ownerId), category as any);
  ...
  public update = async (req: Request, res: Response): Promise<void> => {
    try {
      const updated = await service.updateDocument(String(req.params.id), req.body);
